...
class CreateAccountActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        accountCreateActBtn.setOnClickListener {
            var email = accountEmailEditText.text.toString().trim()
            var password = accountPasswordEditText.text.toString().trim()
            var displayName = accountDisplayNameEditText.text.toString().trim()
                ...
                if (task.isSuccessful) {
                    ...
                    mDatabase!!.setValue(userObject).addOnCompleteListener { task: Task<Void> ->
                        if (task.isSuccessful) {
                            ...
                            Toast.makeText(this,"There was an error creating user", Toast.LENGTH_LONG).show()
